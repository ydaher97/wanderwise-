...
'use server';
/**
 * @fileOverview A Genkit tool for finding place suggestions (restaurants, attractions) with coordinates.
 */
...
const FindPlacesOutputSchema = z.array(
  z.object({
    ...
    imageUrl: z.string().optional().describe('A URL to an image of the place.'),
  })
).describe('A list of suggested places including their coordinates and image URLs if available.');


export const findPlacesTool = ai.defineTool(
  {
    name: 'findPlacesTool',
    description: 'Fetches suggestions for places like restaurants, tourist attractions, or cafes in a given location, including their coordinates and image URLs if available. Use this to find specific establishments to include in the itinerary. Always use the exact name returned by this tool when referring to the place in the itinerary.',

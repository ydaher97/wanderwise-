...
export function TripForm() {
  ...
  const { user } = useAuth(); // Get user for saving itinerary
  const { setItineraryData, setIsLoading, setError, clearItinerary } = useItineraryStore();
  ...
  const form = useForm<z.infer<typeof GenerateItineraryInputSchema>>({
    resolver: zodResolver(GenerateItineraryInputSchema),
    defaultValues: {
      destination: "",
      startDate: new Date(),
  ...
  useEffect(() => {
    ...
    const loader = new Loader({
      apiKey: mapsApiKey,
      version: "weekly",
      libraries: ["maps", "marker", "places"], // Standardized order
    });
  ...
  async function onSubmit(values: z.infer<typeof GenerateItineraryInputSchema>) {
    setIsLoading(true);
    clearItinerary(); 
    ...
    try {
      ...
      if (user) {
        const itineraryToSave: NewItineraryData = {
          name: `${values.destination} - ${format(values.startDate, "MMM yyyy")}`, // Auto-generated name
          ...
          itineraryText: result.itinerary,
        };
        try {
      ...
      setItineraryData({
        id: itineraryId || undefined, // Pass ID if saved
        itineraryText: result.itinerary,
        destination: values.destination,
        budget: values.budget,
        startDate: formattedStartDate,
        ...
        numberOfPeople: values.numberOfPeople,
        preferences: values.preferences,
        createdAt: itineraryId ? new Date().toISOString() : undefined, // Approximate createdAt for newly generated
      });
